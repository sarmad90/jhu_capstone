(function() {
  ...
  angular
    .module("jhu_capstone.subjects")
    .factory("jhu_capstone.subjects.Image", ImageFactory);

  ImageFactory.$inject = ["$resource", "jhu_capstone.config.APP_CONFIG"];
  function ImageFactory($resource, APP_CONFIG) {
    var service = $resource(APP_CONFIG.server_url + "/api/images/:id",
      { id: "@id" },
      { update: { method: "PUT" }, save: { method: "POST", transformRequest: checkEmptyPayload } });
